...
| 2 | `docs/architecture/system-overview.md` | 单页蓝图，`[SO-Map]` 聚合 Goal Tree → Stage D → QA 链，记录 `Last Synced Goal Tree` 与 `goal_tree_sync` 日志。 | 每次 `goal_tree_sync` 生成新 log（现为 `tests/xi.Core.Tests/Fixtures/Reports/goal-tree-sync-20251118-192655.log`）或 `system-overview` header 时间戳变动时更新。 |
...
| 4 | `scripts/goal_tree_sync.py` + `tests/xi.Core.Tests/Fixtures/Reports/goal-tree-sync-*.log` | Goal Tree → `port-blueprint.md`/`m3-implementation-plan.md` 同步器；log 记录写入的文件。 | 新 log 文件（最新 `goal-tree-sync-20251118-192655.log`）/ `generated-at` 注释漂移；若 `DRIFT:` 涉及 Stage D anchors，通知架构师+Rust Porter。 |
...
|10 | `tests/xi.Core.Tests/Fixtures/fixtures.manifest.json` · `tests/xi.Core.Tests/Fixtures/Reports/{stage-d-inspector-latest.txt,chunk-bench-latest.txt,grapheme-telemetry*.txt,.trx,stage-d-refresh-*.log}` | Stage D 证据集合：`rust_commit=3799d2be9db0ef040517ed69df1b717e96a8958e`、Chunk吞吐 255.93 MB/s、Grapheme telemetry 12k ops（0.30% fallback）、`stage-d-refresh-20251118-191129.log` 记录 exporter→loader→hydrator→manifest→Inspector 全链。 | 新日志/哈希或 `feature_gates` 发生变化时更新；缺任一文件即视为证据链断裂。 |

## 监控清单
| 监控对象 | 信号/检测手段 | 触发时动作 | 频率 |
|-----------|---------------|-------------|------|
| Goal Tree 模板 `docs/architecture/templates/goal-tree.yaml` | `sha256sum` 与 `python scripts/goal_tree_sync.py --check`（当前 log `goal-tree-sync-20251118-192655.log`）的 `PLAN/DRIFT` 输出 | 记录 hash + log，在本档案“证据快照”更新引用；若 `DRIFT` 涉及 Stage D，立即 ping 架构师。 | 每日或模板 PR 后 |
...
| Stage D manifest + inspector (`fixtures.manifest.json`, `Reports/stage-d-inspector-latest.txt`) | `rust_commit`、`feature_gates`、 ledger payload hash（当前 `cursor_state, serde, tree_builder_slice_trace` + hashes `69ba7f25`/`9b46bd8e`/`eb0c7c66`/...） | 若任意 hash 变化或 `feature_gates` 缺少 tree trace，提醒 Rust Porter + QA 并记录 log 路径。 | 每次 Stage D refresh |
| Stage D orchestrator日志 (`Reports/stage-d-refresh-*.log`, `python scripts/refresh_all_assets.py --only stage-d-fixtures`) | 退出码、命令序列是否包含 exporter/loader/hydrator/verify/bench/telemetry（最新记录：`stage-d-refresh-20251118-191129.log` 全链） | 如日志缺段或命令跳过默认 flags，参考 `docs/operations/do-check-stage-d.md` 打回执行人并补档。 | 每次脚本运行 |
...
## 当前关注（Current Focus）
- 维持 Goal Tree 模板 → `goal_tree_sync` log → `port-blueprint` / `m3-implementation-plan` / `rope-port-mapping` 的一致性，当前校验日志为 `tests/xi.Core.Tests/Fixtures/Reports/goal-tree-sync-20251118-192655.log`（手动更新 `generated-at` 注释以配合 `--check` 输出）。
- 守护 Stage D 证据链：`fixtures.manifest.json`、`stage-d-inspector-latest.txt`、`chunk-bench-latest.txt`、`grapheme-telemetry-20251118T191914Z.txt`/`.trx`、`stage-d-refresh-20251118-191129.log`，确保 `[StageD::ParityAssets]` 与 `[QA-*]` anchors 复用同一批 artefact。
- 追踪 `docs/operations/do-check-stage-d.md` “Latest Evidence” + checklist 与 `docs/architecture/system-overview.md#[SO-Map]` 的 Stage D 行，保证下一次 `python scripts/refresh_all_assets.py --only stage-d-fixtures` 引用仍可回溯到同一证据索引。

## 证据快照（2025-11-19）
- `tests/xi.Core.Tests/Fixtures/fixtures.manifest.json`：`rust_commit=3799d2be9db0ef040517ed69df1b717e96a8958e`，`cli_revision=0.3.0`，`feature_gates=[cursor_state, serde, tree_builder_slice_trace]`，10 个 ledger（chunk/cursor/grapheme/breaks/diff/search/tree trace + serde baselines）全部匹配 `python scripts/verify_fixture_manifest.py` 校验报告。
- `tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt`：Chunk=20 (`payload=69ba7f2536…`)，Cursor=12 (`9b46bd8e…`)，Grapheme=668 (`eb0c7c66…`)，Breaks/Diff/Search 各 3（hash=`5d37a731…`/`fe76ed31…`/`7eac7ecf…`），Tree trace `basic_slice_plan.json`（3 events，`22724af7…`）；wrap span 24–38，所有计数与 manifest 一致。
- `tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`：Release bench 指向同 manifest，chunk throughput 255.93 MB/s、line throughput 347.07 MB/s、线程分配 37,944 B，`Inspector note` 回链到 `stage-d-inspector-latest.txt`。
- `tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry-20251118T191914Z.txt` + `grapheme-telemetry.trx`：12,000 operations / 6,000 descriptor replays / 0 neighbour requests / fallback 0.30%，TRX（`Xi.Core.Tests.GraphemeNavigatorSmokeTests`) 3/3 Passed（`duration≤28 ms`），时间戳 `2025-11-19T03:13:13+08:00`。
- `tests/xi.Core.Tests/Fixtures/Reports/stage-d-refresh-20251118-191129.log`：`python scripts/refresh_all_assets.py --only stage-d-fixtures` 串联 Rust `run_all_checks --filter serde-fixtures`、`cargo run export-serde-fixtures --cursor/...--tree-builder-trace`、`dotnet test Xi.Editor.sln`（含 Loader/Hydrator filters）、`python scripts/verify_fixture_manifest.py` 以及 StageDDescriptorInspector，所有命令成功并回显 10/10 hash `match`。
- `tests/xi.Core.Tests/Fixtures/Reports/goal-tree-sync-20251118-192655.log`：`python scripts/goal_tree_sync.py --check` 输出当前差异（需人工同步 `generated-at` 元数据）；`port-blueprint.md`、`m3-implementation-plan.md`、`rope-port-mapping.md` 已将 `Last Synced Goal Tree` 更新到该 log。
...
## 待办 / 风险
- [TODO] 将本次 evidence index（`stage-d-refresh-20251118-191129.log` 等）逐条串进 `[StageD::ParityAssets]`、`[QA-IngestionSmoke]`、`[QA-ChunkBench]`、`[QA-Telemetry]`，避免 anchors 继续引用旧日志。
- [TODO] 追踪下一次 `python scripts/goal_tree_sync.py --update`（带 manifest/inspector/chunk 参数），让 `goal_tree_sync-20251118-192655.log` 之后的 `generated-at` 字段由脚本写入而不是人工维护。
- [TODO] 协调 QA Engineer 在下一个 Stage D rerun 中收集 StageDDescriptorInspector `--report`（或 tee 输出）以及新的 chunk bench / telemetry，以便 `Latest Evidence` 段落获得自动化轮替。
