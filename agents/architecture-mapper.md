...
## 当前聚焦
- 11/21 前统一 Goal Tree / Stage D anchors（A1）：以最新 `tests/xi.Core.Tests/Fixtures/fixtures.manifest.json`（Rust commit `3799d2be9db0ef040517ed69df1b717e96a8958e`, feature gates `["cursor_state","serde"]`）、`tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt` 与 `tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt` 为证据，先在 `docs/architecture/templates/goal-tree.yaml` 填入 manifest/hash/link，再运行 `python scripts/goal_tree_sync.py --check --update`（脚本若未启用则手工同步）确保 `port-blueprint.md#[BP-GoalTree]` 与 `m3-implementation-plan.md#[MP-GoalTree]` 获得 2025-11-21+ 的 `goal-tree:meta` 与 QA/Stage D 列链接，并记录命令/sha256 以备审计。
