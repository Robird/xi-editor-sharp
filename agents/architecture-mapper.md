...
- **2025-11-19 – Sprint 1 Ready Queue 证据链复查**：`docs/sprints/sptrint-1.md` Ready Queue 补上 “实际完成日期/关键输出路径” 列，并在 #1/#2/#5/#6/#7 的验收标准中追加 `tests/xi.Core.Tests/Fixtures/Reports/*.log` 引用；`Coordination Notes` 新增 “Completion Broadcast” 记录每个任务通知到的角色（指向 `AGENTS.md#工作日志`、`docs/meetings/2025-11-19-knowledge-refresh-chat.md`）。同时在 `docs/architecture/system-overview.md#[SO-StageDQAEvidence]` 描述 Stage D→QA→Evidence 三段链路，确保 Evidence index (`agents/information-researcher.md#证据快照` / `docs/operations/do-check-stage-d.md`) 与 QA anchors 互相映射。
