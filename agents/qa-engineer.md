...
- **2025-11-20**：`dotnet run --project tests/xi.Core.Tests/Benchmarks/Diagnostics/RopeChunkEnumeratorBenchmarks.csproj -- --stage-d --report tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt` 在最新 Stage D manifest 上回放 chunk/line：`Chunk samples=9`, `Chunks=3`, `MaxChunkLength=753`, `TotalUtf16Chars=1,808`, `LinesVisited=8/11`，Chunk 4.85 ms（`1MB / 4.85ms ≈ 216 MB/s`）、Line 3.62 ms（≈289 MB/s）均高于 200 MB/s 阈值；完整日志写入 `tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`。下一步：加入 `-c Release --no-build` 与 alloc 计数（<5 MB）模式，锁定 `[QA-ChunkBench]` 的长期监控。
...
- **Stage D ingestion smoke `[QA-IngestionSmoke]`**：🔁 2025-11-20 run；`python scripts/refresh_all_assets.py --only stage-d-fixtures` 导出的 manifest 记录 `rust_commit=3799d2be9db0ef040517ed69df1b717e96a8958e`、`feature_gates=["cursor_state","serde"]`，`python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json` 输出 “All 9 fixtures match the manifest hashes.”，`dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures | tee tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt` 更新 inspector 摘要，ledger payload 更新为 chunk `69ba7f25...`、grapheme `eb0c7c66...`、breaks `5d37a731...`、diff `fe76ed31...`、search `7eac7ecf...`。`dotnet test Xi.Editor.sln --filter "StageDDescriptorLoaderTests|StageDDescriptorHydratorTests"` 中 Hydrator 套件通过，但 Loader 仍期待 `bd28...` ledger，pipeline 以 exit code 1 结束；等待 C# Implementer 更新断言后（目标 2025-11-21）再 rerun 同一命令恢复绿灯。
- **1 MB chunk benchmark `[QA-ChunkBench]`**：2025-11-20 在 Stage D 模式运行 `dotnet run --project tests/xi.Core.Tests/Benchmarks/Diagnostics/RopeChunkEnumeratorBenchmarks.csproj -- --stage-d --report tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`，`Chunk samples=9`, `Chunks=3`, `MaxChunkLength=753`, `TotalUtf16Chars=1,808`, `LinesVisited=8/11`，Chunk/Line 分别 4.85 ms / 3.62 ms（约 216 MB/s / 289 MB/s，满足 ≥200 MB/s）。下一步换用 `-c Release --no-build -- --stage-d --report ... --include-alloc-stats` 以记录 <5 MB alloc，并把结果反映到 `[QA-ChunkBench]`、`m3-implementation-plan.md §5.3`。
...
| Bench diag | ```dotnet run --project tests/xi.Core.Tests/Benchmarks/Diagnostics/RopeChunkEnumeratorBenchmarks.csproj -- --stage-d --report tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt``` | 🔁 2025-11-20：Chunk 4.85 ms（≈216 MB/s），Line 3.62 ms（≈289 MB/s），`Chunk samples=9`, `Chunks=3`, `LinesVisited=8/11` | Stage D 模式吞吐达标；下一步改为 `-c Release --no-build -- --stage-d --report ... --include-alloc-stats` 以补 <5 MB alloc 证据 |

## Stage D & QA Anchors
| Anchor | 目标 | 当前状态 | 关联文档 |
| --- | --- | --- | --- |
| `[QA-IngestionSmoke]` | `scripts/refresh_serialization_fixtures.ps1` 全链路 + manifest 比对 + ingestion smoke | ⚠️ 2025-11-20：`python scripts/refresh_all_assets.py --only stage-d-fixtures` + `python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json` + `dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures | tee tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt` 已写入 Rust commit `3799d2be9db0ef040517ed69df1b717e96a8958e` 的新 ledger（chunk `69ba7f25...`、grapheme `eb0c7c66...`、breaks `5d37a731...`、diff `fe76ed31...`、search `7eac7ecf...`），但 `dotnet test Xi.Editor.sln --filter "StageDDescriptorLoaderTests|StageDDescriptorHydratorTests"` 仍因 Loader 期待旧 `bd28...`/`f0c9...`/`0a82...` 断言而失败；等待测试常量更新后 rerun（目标 2025-11-21）。 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `StageD::FixtureFlow` |
| `[QA-ChunkBench]` | 1 MB chunk/line 架构基准，输出 `ChunkCount/LinesCount/MaxChunkLength/Duration` | 🔁 2025-11-20：`dotnet run --project tests/xi.Core.Tests/Benchmarks/Diagnostics/RopeChunkEnumeratorBenchmarks.csproj -- --stage-d --report tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`（Chunk 4.85 ms≈216 MB/s，Line 3.62 ms≈289 MB/s，`Chunk samples=9`, `MaxChunkLength=753`, `LinesVisited=8/11`），吞吐达标；待补 `-c Release --no-build -- --include-alloc-stats` 日志验证 <5 MB alloc。 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `docs/architecture/m3-implementation-plan.md §5.3` |
...
- [ ] **1 MB Chunk Bench Release + alloc 记录**：2025-11-20 Stage D 模式 chunk bench 已达到 4.85 ms / 3.62 ms（≈216/289 MB/s），仍需 `-c Release --no-build -- --stage-d --include-alloc-stats` 跑分并捕获 <5 MB alloc 证据才能关闭 `[MP-R10]`。
