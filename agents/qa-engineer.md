...
## 当前监控
- **11/27 QA readiness checkpoint**：三条 QA anchors（`[QA-IngestionSmoke]`、`[QA-ChunkBench]`、`[QA-Telemetry]`）须在 11/23-11/24 之前更新到 2025-11-20+ 产出，供 11/27 Goal Alignment 回顾；所有日志、TRX、报告需落盘于 `tests/xi.Core.Tests/Fixtures/Reports/`。
- **Stage D Sync Plan（`docs/meetings/2025-11-20-type-mapping-sync-chat.md#qa-engineer`）**：跟踪 `QA-A1`（`python scripts/refresh_all_assets.py --only stage-d-fixtures` 留档 + `StageDDescriptorInspector` 摘要）、`QA-A2`（`dotnet run --project tests/xi.Core.Tests/Benchmarks/Diagnostics/RopeChunkEnumeratorBenchmarks.csproj -c Release --no-build -- --include-alloc-stats`，补齐 >200 MB/s 与 <5 MB alloc 指标）与 `QA-A3`（`dotnet test tests/xi.Core.Tests/xi.Core.Tests.csproj -v m --filter GraphemeNavigatorSmokeTests --logger "trx;LogFileName=grapheme.trx"`，刷新 `[QA-Telemetry]` hit-rate ≤0.5%）。待 Rust Porter 提供 Breaks/Diff/Search 真 payload 后更新 `[QA-IngestionSmoke]` 与 `docs/architecture/m3-implementation-plan.md §5.3` QA 表。
- **Stage D ingestion smoke `[QA-IngestionSmoke]`**：本日复查 `python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json`（All 9 match）及 `dotnet test Xi.Editor.sln --filter StageDDescriptor{Loader|Hydrator}Tests` 均通过，证明 Loader 已同步 `69ba7f25/eb0c7c66/5d37a731/fe76ed31/7eac7ecf` ledger；缺口是 `python scripts/refresh_all_assets.py --only stage-d-fixtures` 尚未在该哈希上重跑，因此 `[QA-IngestionSmoke]` 仍没有 exit code=0 的“Stage D steps completed”日志可引用。
- **1 MB chunk benchmark `[QA-ChunkBench]`**：`tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`（2025-11-18T10:29:10Z）记录 `Chunk samples=9`, `Chunks=3`, `MaxChunkLength=753`, `LinesVisited=8/11`，Chunk 3.67 ms / Line 2.86 ms（Stage D Debug 配置，无 alloc 统计）。文档仍引用旧的 4.85 ms / 3.62 ms 数据，需要 rerun `dotnet run --project tests/...RopeChunkEnumeratorBenchmarks.csproj -c Release --no-build -- --stage-d --include-alloc-stats --report ...` 并把 Release 吞吐 + <5 MB alloc 写入 `[QA-ChunkBench]` 与 `m3-implementation-plan.md §5.3`。
- **Grapheme fallback telemetry `[QA-Telemetry]`**：`GraphemeNavigatorSmokeTests` instrumentation ready，需在 2025-11-23 前运行 `dotnet test tests/xi.Core.Tests/xi.Core.Tests.csproj -v m --filter GraphemeNavigatorSmokeTests --logger "trx;LogFileName=tests/xi.Core.Tests/TestResults/grapheme.trx"`，把 fallback ≤0.5% 命中率整理成 `[QA-Telemetry]`/`design-divergence-log.md` 记录；若超标立即升级 `[MP-R10]`。
...
| Bench diag | ```dotnet run --project tests/xi.Core.Tests/Benchmarks/Diagnostics/RopeChunkEnumeratorBenchmarks.csproj -- --stage-d --report tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt``` | 🔁 2025-11-18：Chunk 3.67 ms（≈286 MB/s），Line 2.86 ms（≈367 MB/s），`Chunk samples=9`, `Chunks=3`, `LinesVisited=8/11`（Debug，无 alloc） | Stage D 模式吞吐达标；下一步改为 `-c Release --no-build -- --stage-d --include-alloc-stats --report ...` 以补 <5 MB alloc 证据 |

## Stage D & QA Anchors
| Anchor | 目标 | 当前状态 | 关联文档 |
| --- | --- | --- | --- |
| `[QA-IngestionSmoke]` | `scripts/refresh_serialization_fixtures.ps1` 全链路 + manifest 比对 + ingestion smoke | ⚠️ 2025-11-18：`python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json`（All 9 match）与 `dotnet test Xi.Editor.sln --filter StageDDescriptor{Loader|Hydrator}Tests`（全绿）佐证 Loader/Hydrator 已指向 `3799d2be…` ledger，但 `python scripts/refresh_all_assets.py --only stage-d-fixtures` 尚未在该哈希上产出 exit code=0 的完整 run log；需 rerun 并把 stdout 与 `tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt` 一并挂载到 `[QA-IngestionSmoke]`。 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `StageD::FixtureFlow` |
| `[QA-ChunkBench]` | 1 MB chunk/line 架构基准，输出 `ChunkCount/LinesCount/MaxChunkLength/Duration` | 🔁 `tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`（2025-11-18 Debug）记录 Chunk 3.67 ms（≈286 MB/s）、Line 2.86 ms（≈367 MB/s），`Chunk samples=9`, `MaxChunkLength=753`, `LinesVisited=8/11`，但无 Release/alloc 数据；需使用 `-c Release --no-build -- --stage-d --include-alloc-stats` rerun 并把 <5 MB alloc 与吞吐落回 `[QA-ChunkBench]`。 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `docs/architecture/m3-implementation-plan.md §5.3` |
| `[QA-Telemetry]` | Grapheme fallback hit-rate + 失效样本留档 | ⚠️ `tests/xi.Core.Tests/Fixtures/Reports/` 下暂无任何 `grapheme-*.trx`/telemetry artefact；待运行 `dotnet test tests/xi.Core.Tests/xi.Core.Tests.csproj -v m --filter GraphemeNavigatorSmokeTests --logger "trx;LogFileName=tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry.trx"` 后把 hit-rate ≤0.5% 的结果写入 `[QA-Telemetry]` 与 `design-divergence-log.md`。 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `docs/architecture/design-divergence-log.md` |
...
- [ ] **1 MB Chunk Bench Release + alloc 记录**：`tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt` 仅有 2025-11-18 Debug run（Chunk 3.67 ms / Line 2.86 ms，缺 alloc 字段），需以 `-c Release --no-build -- --stage-d --include-alloc-stats` 重跑并捕获 <5 MB alloc 证据才能关闭 `[MP-R10]`。
