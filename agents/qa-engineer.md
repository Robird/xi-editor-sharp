...
- **Stage D ingestion smoke `[QA-IngestionSmoke]`**：🔁 2025-11-18 run；`python scripts/refresh_all_assets.py --only stage-d-fixtures` 默认串联 loader + hydrator + verifier（`python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json` 输出 “All 9 fixtures match the manifest hashes.”）+ inspector（`dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures`），本次 manifest 记录 `rust_commit=bd28ebdf83d2dd2fa6d4bd7c857d2471a21b4999`、`feature_gates=["cursor_state","serde"]`，ledger payload：chunk `f0c9ada5…`、grapheme `53701cb6…`、breaks `0a82a69f…`、diff `2c4c366c…`、search `e7de44d7…`。`dotnet test Xi.Editor.sln --filter StageDDescriptor` 与 `--filter Serialization` 绿灯；Playbook `[StageD::StageDChecklist]` / `[StageD::ParityAssets]` / `[QA-IngestionSmoke]` 已同步该证据链。
...
| Anchor | 目标 | 当前状态 | 关联文档 |
| --- | --- | --- | --- |
| `[QA-IngestionSmoke]` | `scripts/refresh_serialization_fixtures.ps1` 全链路 + manifest 比对 + ingestion smoke | 🔁 2025-11-18：`python scripts/refresh_all_assets.py --only stage-d-fixtures` 串联 loader/hydrator/verifier/inspector，`python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json` 输出 “All 9 fixtures match the manifest hashes.”，`dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures` 记录 Breaks/Diff/Search 各 3 样本与新 hash（chunk `f0c9ada5…`、grapheme `53701cb6…`、breaks `0a82a69f…`、diff `2c4c366c…`、search `e7de44d7…`），`dotnet test Xi.Editor.sln --filter StageDDescriptor` 与 `--filter Serialization` 全绿 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `StageD::FixtureFlow` |
