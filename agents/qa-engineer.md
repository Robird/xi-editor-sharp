...
## 当前监控
- **11/27 QA readiness checkpoint**：三条 QA anchors 已在 2025-11-18 rerun（`stage-d-refresh-20251118-143204.log`, `chunk-bench-latest.txt`, `grapheme-telemetry.trx`），下一次落盘需在 11/23–11/24 之间完成以供 11/27 Goal Alignment 复核。
- **Stage D Sync Plan（`docs/meetings/2025-11-20-type-mapping-sync-chat.md#qa-engineer`）**：`QA-A1`（Stage D orchestrator + inspector log）、`QA-A2`（Release chunk bench + alloc stats ≥200 MB/s / <5 MB）、`QA-A3`（Grapheme telemetry ≤0.5% fallback）均已于 2025-11-18 提交 artefact；等待 Rust Porter 更新 Breaks/Diff/Search payload 后再触发下一轮。
- **Stage D ingestion smoke `[QA-IngestionSmoke]`**：✅ `python scripts/refresh_all_assets.py --only stage-d-fixtures` 于 2025-11-18 产出完整流水线（loader/hydrator/manifest verifier/inspector + Release bench/telemetry），日志 `tests/xi.Core.Tests/Fixtures/Reports/stage-d-refresh-20251118-143204.log` 与 `stage-d-inspector-latest.txt` 已挂接 Playbook。
- **1 MB chunk benchmark `[QA-ChunkBench]`**：✅ `tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`（Release，4.97 ms / 3.06 ms，201.25 MB/s / 327.31 MB/s，线程分配 37,944 bytes，GC 0/0/0）满足吞吐与 alloc 阈值；`Program.cs` 现默认输出吞吐字段，便于 Goal Tree 校验。
- **Grapheme fallback telemetry `[QA-Telemetry]`**：✅ `tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry.trx`（2025-11-18）记录 `GraphemeNavigatorSmokeTests` 命令，metrics snapshot 显示 Forward/Backward neighbor 各 1 次、`scalarFallbacks=0`，fallback ratio 0%；`docs/csharp-refactor/rope-serialization-fixture-playbook.md` 已引用。
- **Stage D Baseline**：Playbook 的 `[QA-IngestionSmoke]` / `[QA-ChunkBench]` / `[QA-Telemetry]` 均更新至 2025-11-18 artefact；持续跟踪 `fixtures.manifest.json` hash 变化与 `stage-d-inspector-latest.txt` ledger。
...
| Bench diag | ```dotnet run --project tests/xi.Core.Tests/Benchmarks/Diagnostics/RopeChunkEnumeratorBenchmarks.csproj --configuration Release -- --stage-d --include-alloc-stats --report tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt``` | ✅ 2025-11-18：Chunk 4.97 ms（201.25 MB/s），Line 3.06 ms（327.31 MB/s），线程分配 37,944 bytes，GC 0/0/0 | 日志 `tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`；吞吐/alloc 现同步至 `[QA-ChunkBench]` |

## Stage D & QA Anchors
| Anchor | 目标 | 当前状态 | 关联文档 |
| --- | --- | --- | --- |
| `[QA-IngestionSmoke]` | `scripts/refresh_serialization_fixtures.ps1` 全链路 + manifest 比对 + ingestion smoke | ✅ 2025-11-18：`python scripts/refresh_all_assets.py --only stage-d-fixtures`（日志：`tests/xi.Core.Tests/Fixtures/Reports/stage-d-refresh-20251118-143204.log`）产出 exporter + loader + hydrator + manifest verifier + inspector + Release bench/telemetry整链；`stage-d-inspector-latest.txt` 与 manifest hash（chunk `69ba7f25`, grapheme `eb0c7c66`, breaks `5d37a731`, diff `fe76ed31`, search `7eac7ecf`）已与 `StageDDescriptorLoaderTests` 断言一致。 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `StageD::FixtureFlow` |
| `[QA-ChunkBench]` | 1 MB chunk/line 架构基准，输出 `ChunkCount/LinesCount/MaxChunkLength/Duration` | ✅ `dotnet run ... --include-alloc-stats --report tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`（2025-11-18 Release）记录 Chunk 4.97 ms（201.25 MB/s）、Line 3.06 ms（327.31 MB/s）、线程分配 37,944 bytes、GC 0/0/0；文档 & Goal Tree 已回填。 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `docs/architecture/m3-implementation-plan.md §5.3` |
| `[QA-Telemetry]` | Grapheme fallback hit-rate + 失效样本留档 | ✅ `dotnet test tests/xi.Core.Tests/xi.Core.Tests.csproj --filter GraphemeNavigatorSmokeTests --logger "trx;LogFileName=tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry.trx"`（2025-11-18）生成 TRX，metrics snapshot：MoveNext=2, MovePrevious=2, Forward/Backward neighbor hits=1/1, ScalarFallbacks=0（fallback ratio 0%）。 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `docs/architecture/design-divergence-log.md` |
