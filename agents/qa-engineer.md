...
- **Stage D ingestion smoke `[QA-IngestionSmoke]`**：🔁 2025-11-19 run；`python scripts/refresh_all_assets.py --only stage-d-fixtures` 默认串联 loader + hydrator + verifier + inspector，日志含 `python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json`（All 9 fixtures match）与 `dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures`（Breaks/Diff/Search 各 3 样本、sha256 报告），manifest 记录 `rust_commit=bea8a3360131a0840d26aa75daba9184b70d50b7`、`feature_gates=["serde"]`。`dotnet test Xi.Editor.sln --filter StageDDescriptor` 与 `--filter Serialization` 绿灯；Playbook `[StageD::StageDChecklist]` / `[StageD::ParityAssets]` hash 与 QA 档案一致。必要时仍可通过 `--update` 写回并 rerun 取证。
...
| Anchor | 目标 | 当前状态 | 关联文档 |
| --- | --- | --- | --- |
| `[QA-IngestionSmoke]` | `scripts/refresh_serialization_fixtures.ps1` 全链路 + manifest 比对 + ingestion smoke | 🔁 2025-11-19：`python scripts/refresh_all_assets.py --only stage-d-fixtures` 串联 loader/hydrator/verifier/inspector，`python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json` 输出 “All 9 fixtures match the manifest hashes.”，`dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures` 记录 Breaks/Diff/Search 各 3 样本，`dotnet test Xi.Editor.sln --filter StageDDescriptor` 与 `--filter Serialization` 全绿 | `docs/csharp-refactor/rope-serialization-fixture-playbook.md`, `StageD::FixtureFlow` |
