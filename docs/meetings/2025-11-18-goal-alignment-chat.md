...
- **差距/风险**：
  ...
  - `tree_builder_slice_trace` asset 现已纳入脚本默认导出；若为调试暂时关闭 `-SkipTreeTrace`，必须在 QA 档案记录原因，并在下一次 Stage D run 中恢复默认，以免 manifest/hash 漂移未被追踪。
- **下阶段计划**：
  ...
  - **11/22（完成 2025-11-18）**：Tree trace 已并入 `ExportParityFixtures` 默认证路；脚本会默认启用 `tree_builder_slice_trace` feature 与 `--tree-builder-trace`，若需暂避请使用 `-SkipTreeTrace` 并在 QA 档案留痕。下一步是 rerun `python scripts/refresh_all_assets.py --only stage-d-fixtures` 以捕获包含 Tree trace 的完整日志。
- **文档/资产更新**：
