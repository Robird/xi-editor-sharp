...
> **最新 manifest（2025-11-18 20:30 UTC Stage D 批次）**：顺序执行 `python scripts/refresh_all_assets.py --only stage-d-fixtures --continue-on-error`, `dotnet test Xi.Editor.sln --filter StageDDescriptor`, `python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json`, `dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures | tee tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt`，把完整流水线写入 `tests/xi.Core.Tests/Fixtures/Reports/stage-d-refresh-20251118-203039.log`。该日志记录 exporter → StageDDescriptorLoaderTests/StageDDescriptorHydratorTests → manifest verifier（“All 10 fixtures match the manifest hashes.”）→ inspector ledger，针对 `rust_commit=b6fb5999288946daeeadc4b7f9f9756f6dda1f50`, `cli_rev=0.3.0`, `feature_gates=["cursor_state","serde","tree_builder_slice_trace"]`，确认 chunk `9c7163ebd6ceb7f9e7e4ec566682470e96e27424499c4a6f5804898eca48f1f0`, grapheme `b3484eab43303735c754fa4fb6928255ef7dbe053d2cf41ce60a7c63a7a59095`, breaks `527ca4f2000b7548a10fe94cf51f6e758f23fa73e7f1ce82347ce92ab61a1f83`, diff `afc04b228f19c7663f1d43597973c0afc90a34159b64ccaf7b9407e6613811a6`, search `599f25b06a36d417cd1ca51a818c0ac5dfcf162c9530368d62cd7075c840df51`, tree trace `22724af7fe8b22e4e1dbd01f86ba1b3902a0ccf777944b5b29081259927c3a6e`, cursor `9b46bd8e29042e38c36556afc4054a5a2c4a6cfa405b5bbd738ca1909f8a4d38`, delta `59c45336bace174a9ab50cefdef05bdd2890d4d93a376df4e6897ca71a20cf7f`, engine `8707d5de24e9369bf3a818a40030626118da2752ee1bdb54363cfb3e1ffa1cc2`, subset `28fa3c807f83961f6cdf9695f3605bdf22972f734e2470adae1234c85deb138f`。同批次 `stage-d-fixtures` 步骤在 Release + `--include-alloc-stats` 模式重放 chunk bench（Chunk 5.51 ms ⇒ 181.45 MB/s、Line 4.21 ms ⇒ 237.40 MB/s、Thread Alloc 37,944 bytes）并刷新 `tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry-latest.txt`（12,000 ops、Requires fallback 0.3000%、Scalar fallback 0%、≈35.8k ops/s）和 `tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry.trx`。鉴于 StageDDescriptorInspector 仍缺失 `--report` flag，QA 继续使用 `tee` 将 stdout 写入 `stage-d-inspector-latest.txt`，Ready Queue #5/#7 仍追踪该 CLI 缺口；无参 orchestrator 运行会自动生成同套 artefact。 
...
| Asset | Path | Export Flag | Schema / Version | SHA256（2025-11-18 20:30 UTC 刷新） | 备注 |
| --- | --- | --- | --- | --- | --- |
...
| Cursor Descriptors | `tests/xi.Core.Tests/Fixtures/cursor_descriptors/cursor_descriptors.json` | `--cursor-descriptors` | `cursor_descriptors@1.2.0` | `9b46bd8e29042e38c36556afc4054a5a2c4a6cfa405b5bbd738ca1909f8a4d38` | `[MP-T1]` NodeCursor parity；manifest 记录 `count=12`，`feature_gates` 现包含 `cursor_state,serde,tree_builder_slice_trace`，Inspector ledger（`stage-d-inspector-latest.txt`）复诵同一 hash。 |
| Chunk Descriptors | `tests/xi.Core.Tests/Fixtures/chunk_descriptors/chunk_descriptors.json` | `--chunk-descriptors` | `chunk_descriptors@1.0.0` | `9c7163ebd6ceb7f9e7e4ec566682470e96e27424499c4a6f5804898eca48f1f0` | `[MP-T3]` Chunk/Line 样本；2025-11-18 20:30Z QA run 通过 StageDDescriptorInspector ledger + Release chunk bench（5.51 ms/4.21 ms，181.45/237.40 MB/s，alloc 37,944 B）重新确认。 |
| Grapheme Descriptors | `tests/xi.Core.Tests/Fixtures/grapheme_descriptors/grapheme_descriptors.json` | `--grapheme-descriptors` | `grapheme_descriptors@1.0.0` | `b3484eab43303735c754fa4fb6928255ef7dbe053d2cf41ce60a7c63a7a59095` | `[MP-T4]` Grapheme fallback 遥测；count=668，hash 由 manifest/Inspector + `grapheme-telemetry-latest.txt`（12,000 ops，fallback 0%）共同背书。 |
| Breaks descriptors (soft line metrics) | `tests/xi.Core.Tests/Fixtures/breaks_descriptors/breaks_descriptors.json` | `--breaks-descriptors`（Stage D exporter 默认传入） | `breaks_descriptors@1.0.0` | `527ca4f2000b7548a10fe94cf51f6e758f23fa73e7f1ce82347ce92ab61a1f83` | 3 条 BreakPlan 样本；StageDDescriptorHydrator + Inspector 记录 wrap 宽 24–38，并在 Ready Queue #5 作为 `[TS-B5]` 证据。 |
| Diff region snapshots | `tests/xi.Core.Tests/Fixtures/diff_regions/diff_regions.json` | `--diff-regions`（Stage D exporter 默认传入） | `diff_regions@1.0.0` | `afc04b228f19c7663f1d43597973c0afc90a34159b64ccaf7b9407e6613811a6` | 3 条 diff 快照；Inspector 输出列举 patch id + 每案 4 ops，证明 Rust exporter/manifest/loader 同步。 |
| Search hits & span windows | `tests/xi.Core.Tests/Fixtures/search_spans/search_spans.json` | `--search-spans`（Stage D exporter 默认传入） | `search_spans@1.0.0` | `599f25b06a36d417cd1ca51a818c0ac5dfcf162c9530368d62cd7075c840df51` | 3 组 search span；Inspector 摘要记录 window + hit count（^WARN:.*$, stage, 🚀），hash 对应 manifest ledger。 |
...
> 数据来源：`python scripts/refresh_all_assets.py --only stage-d-fixtures --continue-on-error`（2025-11-18 20:30Z QA run）触发 exporter/loader/hydrator/manifester；`python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json` 输出 “All 10 fixtures match the manifest hashes.”，随后 `dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures | tee tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt` 记录 Breaks/Diff/Search typed 摘要与 SHA。Release chunk bench（5.51 ms/4.21 ms，181.45/237.40 MB/s，alloc 37,944 B）和 Grapheme telemetry（12,000 ops，fallback 0%，ops/sec ≈35.8k）同批写入 `chunk-bench-latest.txt`、`grapheme-telemetry-latest.txt` 与 `.trx`，供 QA anchors 共用；无参 `python scripts/refresh_all_assets.py` 现已默认执行等价的 `stage-d-fixtures` 子流程。
...
> **Manifest（2025-11-18 20:30Z 刷新）**：`python scripts/refresh_all_assets.py --only stage-d-fixtures --continue-on-error` 写入 `fixtures.manifest.json`，记录 `rust_commit=b6fb5999288946daeeadc4b7f9f9756f6dda1f50`、`cli_rev=0.3.0`、`feature_gates=["cursor_state","serde","tree_builder_slice_trace"]`，descriptor 计数为 `chunk=20` / `cursor=12` / `grapheme=668`，`breaks/diff/search` 各 3 条。所有 hash 以 manifest 为准，Stage D loader + hydrator + inspector smoke 会在 `[QA-IngestionSmoke]` 中登记；若无特殊需求，直接执行无参 orchestrator即可复现相同 manifest 更新。 
...
> **Latest run — 2025-11-18 20:30 UTC（Stage D ingestion smoke + ledger audit）**：`python scripts/refresh_all_assets.py --only stage-d-fixtures --continue-on-error`, `dotnet test Xi.Editor.sln --filter StageDDescriptor`, `python scripts/verify_fixture_manifest.py --manifest tests/xi.Core.Tests/Fixtures/fixtures.manifest.json`, `dotnet run --project tools/StageDDescriptorInspector -- --fixtures tests/xi.Core.Tests/Fixtures | tee tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt` 这一序列写入 `tests/xi.Core.Tests/Fixtures/Reports/stage-d-refresh-20251118-203039.log`。StageDDescriptorLoaderTests/StageDDescriptorHydratorTests 合计 11 个断言全部通过，manifest verifier 复诵 “All 10 fixtures match the manifest hashes.”，Inspector ledger 重述同一 SHA 集（chunk `9c7163eb...`, grapheme `b3484eab...`, breaks `527ca4f2...`, diff `afc04b22...`, search `599f25b0...`, tree trace `22724af7...`, cursor `9b46bd8e...`, delta `59c45336...`, engine `8707d5de...`, subset `28fa3c80...`）。同批 artefact 还覆盖 `chunk-bench-latest.txt`（5.51 ms / 4.21 ms，181.45 / 237.40 MB/s，alloc 37,944 B）与 `grapheme-telemetry-latest.txt`（12,000 ops，fallback 0%，Requires fallback 0.3000%），并刷新 `grapheme-telemetry.trx`。`[StageD::FixtureFlow]` / `[QA-IngestionSmoke]` 现引用这些路径，为 Rust/C#/QA 提供无需重跑的复核基线。 
...
> **Latest run — 2025-11-18 20:35:14Z（Release chunk/line replay + alloc stats）**：`dotnet run --project tests/xi.Core.Tests/Benchmarks/Diagnostics/RopeChunkEnumeratorBenchmarks.csproj --configuration Release -- --stage-d --include-alloc-stats --report tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt` 重放 manifest ledger `9c7163ebd6ceb7f9e7e4ec566682470e96e27424499c4a6f5804898eca48f1f0`（Rust commit `b6fb5999288946daeeadc4b7f9f9756f6dda1f50`, feature gates `cursor_state,serde,tree_builder_slice_trace`）：chunk 阶段 5.51 ms ⇒ **181.45 MB/s**，line 阶段 4.21 ms ⇒ **237.40 MB/s**，线程分配 37,944 bytes，GC gen0/1/2 = 0。报告写入 `tests/xi.Core.Tests/Fixtures/Reports/chunk-bench-latest.txt`，Inspector note `tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt` 再次复述相同 ledger/metric_windows 证据，继续满足 `[MP-R10]` 的 <5 MB alloc 与 >200 MB/s 吞吐阈值。
...
- **Latest run — 2025-11-18 20:35:19Z（Release harness ≥12k operations）**：`dotnet run --project tests/xi.Core.Tests/Benchmarks/GraphemeTelemetryHarness/GraphemeTelemetryHarness.csproj --configuration Release -- --fixture-dir tests/xi.Core.Tests/Fixtures --target-ops 12000 --report tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry-latest.txt`（Stage D manifest `rust_commit=b6fb5999288946daeeadc4b7f9f9756f6dda1f50`, feature gates `cursor_state,serde,tree_builder_slice_trace`）回放 6,000 descriptors（9 次 pass，实际 12,000 MoveNext/MovePrevious 操作），`Scalar fallback count=0` ⇒ fallback ratio **0.0000%**，`Requires fallback descriptors=18 (0.3000%)`，`Cross-leaf cases=9`，耗时 **335.20 ms**（≈35,800 ops/s）。同批次的 `stage-d-inspector-latest.txt`/`chunk-bench-latest.txt` 共享 manifest ledger；日志路径 `tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry-latest.txt`，TRX 仍由 `dotnet test Xi.Editor.sln --filter Category=StageDTelemetry --logger "trx;LogFileName=tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry.trx"` 提供（total=3/executed=3/passed=3）。
