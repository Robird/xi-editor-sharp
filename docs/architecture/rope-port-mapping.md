...
| --- | --- | --- | --- | --- | --- |
| Node / SharedNode / TreeBuilder | `rope/tree.rs` | `Tree/Node*.cs`, `TreeBuilder.cs`, `Tree/LeafSplitter.cs`, `Tree/StringLeafOperations.cs`, `Tree/TreeBuilderTracer.cs` | Implementing | Rust skeleton exposes `Node<N,L>`, `SharedNode`, `TreeBuilderTracer`, `TreeBuilderEventKind`; C# now ships `TreeBuilderTracer/EventKind/Event/ITreeBuilderTracer` + `NoOpTreeBuilderTracer` placeholders but the slice-trace hook is not yet wired to Stage D. | SharedNode helpers + `_editVersion` + tracer scaffolding all exist; remaining work is injecting the tracer through `TreeBuilder` + `MetricAdapter` (`[TS-B2]`) and replaying Stage D traces once the loader/QA hook is ready. |
...
| Chunk + Line enumerators | `rope/rope.rs::ChunkIter`, `Lines*` | `RopeChunkEnumerator*.cs`, `RopeLineEnumerator*.cs`, `Rope/Diagnostics/Descriptors/ChunkDescriptor.cs`, `Rope/Diagnostics/Descriptors/LineDescriptor.cs` | Diagnostics only | Rust skeleton covers `ChunkIter`, `LinesRaw`, `ChunkDescriptor`, `LineDescriptor` exporter structs; C# now mirrors the descriptor DTOs and manifest schema but still lacks a loader/CLI exporter. | Copy-on-read enumerators + telemetry counters remain the downgrade path; Stage D manifest carries 20 chunk descriptors (hash `bd863f2237dd…`) and C# DTOs mirror them, but `[TS-B3]` now tracks the loader + `fixtures.manifest.json` binding so `[QA-ChunkBench]` can log the 1 MB baseline. |
...
2. **Wire manifest ingestion to QA benches** – Point `[QA-ChunkBench]`/`[QA-IngestionSmoke]` loaders at `fixtures.manifest.json`, using the C# `Chunk/GraphemeDescriptor` DTOs as the ingestion target and persisting telemetry snapshots so Stage D evidence stays reproducible (`[TS-B3]`).
