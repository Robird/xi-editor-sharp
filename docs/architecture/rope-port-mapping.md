...
| Chunk + Line enumerators | `rope/rope.rs::ChunkIter`, `Lines*` | `RopeChunkEnumerator*.cs`, `RopeLineEnumerator*.cs`, diagnostics harness | Diagnostics only | Copy-on-read enumerators + telemetry counters remain the downgrade path; Stage D manifest now carries 20 chunk descriptors (hash `bd863f2237dd…`, refreshed 2025-11-17 via `python scripts/refresh_all_assets.py --only stage-d-fixtures`), so the next gate is wiring `[QA-ChunkBench]` to consume `fixtures.manifest.json` and log the 1 MB baseline. |
| Grapheme navigation | `rope/rope.rs::GraphemeCursor` | `Navigation/DegradedGraphemeNavigator.cs`, `GraphemeNavigationMetrics.cs` | Degraded (monitor) | Surrogate-safe fallback accepted (see `[Div-Active]`); Stage D manifest lists 668 grapheme windows (hash `a2b84031c5aa…`, refreshed 2025-11-17) and `[StageD::ParityAssets]` keeps them tied to CLI rev 0.3.0; next up is capturing iterator CLI traces so `[QA-Telemetry]` can compare Rust vs degraded mode. |
