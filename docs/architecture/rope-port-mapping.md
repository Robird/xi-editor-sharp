...
| --- | --- | --- | --- | --- | --- |
| Node / SharedNode / TreeBuilder | `rope/tree.rs` | `Tree/Node*.cs`, `TreeBuilder.cs`, `Tree/LeafSplitter.cs`, `Tree/StringLeafOperations.cs` | Implementing | Rust skeleton exposes `Node<N,L>`, `SharedNode`, `TreeBuilderTracer`, `TreeBuilderEventKind`; C# covers `Tree/Node*.cs` + leaf ops but lacks tracer/slice-trace types. | SharedNode helpers + `_editVersion` shipped; remaining work is hooking `MetricAdapter` (`[TS-B2]`) before the M4 generic switch and mirroring the `NodeCursorState` docs in `[StageD::FixtureFlow]`. |
...
| Chunk + Line enumerators | `rope/rope.rs::ChunkIter`, `Lines*` | `RopeChunkEnumerator*.cs`, `RopeLineEnumerator*.cs`, diagnostics harness | Diagnostics only | Rust skeleton covers `ChunkIter`, `LinesRaw`, `ChunkDescriptor`, `LineDescriptor` exporter structs; C# only has `RopeChunkEnumerator*` diagnostics (no descriptor DTOs or CLI exporter). | Copy-on-read enumerators + telemetry counters remain the downgrade path; Stage D manifest now carries 20 chunk descriptors (hash `bd863f2237dd…`, refreshed 2025-11-17 via `python scripts/refresh_all_assets.py --only stage-d-fixtures`), so the next gate is wiring `[QA-ChunkBench]` to consume `fixtures.manifest.json` and log the 1 MB baseline. |
| Grapheme navigation | `rope/rope.rs::GraphemeCursor` | `Navigation/DegradedGraphemeNavigator.cs`, `GraphemeNavigationMetrics.cs` | Degraded (monitor) | Rust exposes `unicode_segmentation::GraphemeCursor`, `GraphemeDescriptor`, exporter structs; C# implements degraded navigator + metrics but lacks descriptor DTO/export flow. | Surrogate-safe fallback accepted (see `[Div-Active]`); Stage D manifest lists 668 grapheme windows (hash `a2b84031c5aa…`, refreshed 2025-11-17) and `[StageD::ParityAssets]` keeps them tied to CLI rev 0.3.0; next up is capturing iterator CLI traces so `[QA-Telemetry]` can compare Rust vs degraded mode. |
...
<a id="RPM-Actions"></a>
1. **Automate canonical hash generation + diffing** – Extend `scripts/refresh_all_assets.py` + `scripts/refresh_serialization_fixtures.ps1` to emit/verifiy hashes per `[Fixture-Manifest]`, publish guardrails in `[StageD::ParityAssets]`, and alert when CLI rev/feature gates diverge.
2. **Wire manifest ingestion to QA benches** – Point `[QA-ChunkBench]` and `[QA-IngestionSmoke]` to `fixtures.manifest.json`, recording results + telemetry snapshots so Stage D evidence stays reproducible (`[TS-B3]`).
3. **Capture Grapheme iterator CLI trace** – Add CLI logging to the Rust exporter so `[QA-Telemetry]` can diff the manifest-backed grapheme set against the degraded navigator (`[TS-B4]`).
4. **Publish MetricAdapter design** – `[TS-B2]` still requires a concrete adapter test plus documentation so `RopeNode` can merge in M4; blueprint milestone G6 stays blocked otherwise.
