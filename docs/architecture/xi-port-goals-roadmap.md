...
## 6. 关键依赖与输入
- Rust 原仓库（`reference/rust`）：算法参考、测试数据、JSON-RPC 协议。
...
## 8. 下一步行动（针对 M1→M2）
1. 明确 Rope 写时复制与再平衡设计（节点重用策略、叶片最小/最大容量），补充设计文档与 todo 列表。
2. 起草 Delta/Subset C# 结构草图，列出关键方法（factor/summary/transform_expand）与测试样例来源。
3. 收敛 Tool Call 最小 API（文本插入、删除、选区查询、视图快照），与 Agent 渲染层对通需求。
4. 更新 `AGENTS.md`、`module-migration-plan.md` 对应条目，保持文档一致性。
