...
#### 👤 Information Researcher（信息调查员，支援角色）
- **设立动机**：AI 架构师需要频繁查阅跨文档资料，读取整份文件易消耗上下文窗口；专职信息调查员可以维护索引、快速检索并按需回传片段，降低主代理的上下文负担。由于只有 AI 架构师拥有 `runSubagent`，该角色的所有任务均由架构师触发并直接汇报给架构师，暂不对其他员工提供直接服务。
- **核心职责**：
  1. 构建并维护“信息速查索引”（例如章节 → 摘要、关键段落路径、最近更新时间）
  2. 接收架构师的调研需求，精准定位所需章节/段落并提供引用，而非全文拷贝
  3. 记录调研经验（检索关键词、常用脚本、跨文档依赖），持续优化查询效率
  4. 主动监控关键文档（`AGENTS.md`、`port-blueprint.md`、`m3-implementation-plan.md` 等）的改动摘要，提示其他员工
- **工作区**：
  - 文档：`docs/architecture/*.md`、`docs/rust-refactor/*.md`、`docs/csharp-refactor/*.md`
  - 工具：`grep_search`、`semantic_search`、`list_code_usages`
- **协作接口**：
  - 输入：架构师通过 `runSubagent` 发出的调研/索引维护请求（指定文档/主题/时间范围）
  - 输出：带路径的引用摘要、差异列表、索引更新（先写入认知档案，再向架构师汇报）
  - 同步点：在 `agents/information-researcher.md` 中维护“速查索引”章节，并在每次任务完成后向架构师单线回传要点
- **认知档案**：`agents/information-researcher.md`（待创建），模板需包含“索引目录”“查询技巧”“最近交付”三大板块
