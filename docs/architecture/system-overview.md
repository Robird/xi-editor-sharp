...
| Stage D Diagnostics | Loader/Hydrator/Inspector 组合已经文档化，但 `[Chat-2025-11-20]` + `[RPM-Actions]` 明确 Breaks/Diff/Search 仍是 skeleton，Inspector 日志也未写入 `[QA-IngestionSmoke]`；11/22–11/27 期间需完成 Rust exporter payload、Stage D→QA wiring 与文档回写，才能把 Stage D 行从“流程 ready”切换到“证据 ready”。 | `[StageD::StageDChecklist]` · `[StageD::FixtureFlow]` · `[StageD::ParityAssets]` · `[RPM-Actions]` | Rust Porter + QA Engineer（`../../agents/rust-porter.md` · `../../agents/qa-engineer.md`） |
