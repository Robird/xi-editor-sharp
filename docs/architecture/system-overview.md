...
| Stage D Diagnostics | Stage D 以 `[StageD::StageDChecklist]` → `[StageD::FixtureFlow]` → `[StageD::ParityAssets]` 串起 CLI、脚本与 manifest，并新增 “Descriptor Hydrator” 节点：`StageDDescriptorHydrator` 建立在 `StageDDescriptorLoader` 之上，把 manifest ledger 转成 Breaks/Diff/Search typed 结构供 QA/CLI 复用。所有 Stage D 操作现在必须在 Playbook 中登记 “loader（哈希校验）→ hydrator（ingestion）” 两步流，以便追踪 Stage D anchors。 | `[StageD::StageDChecklist]` · `[StageD::FixtureFlow]` · `[StageD::ParityAssets]` | Rust Porter + QA Engineer（`../../agents/rust-porter.md` · `../../agents/qa-engineer.md`） |
