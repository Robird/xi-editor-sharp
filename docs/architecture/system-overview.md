...
| Stage D Diagnostics | Loader/Hydrator/Inspector 组合在 `tests/xi.Core.Tests/Fixtures/Reports/stage-d-refresh-20251118-191129.log` 记录 `python scripts/refresh_all_assets.py`（无参数）→ exporter → loader/hydrator → manifest 校验 → StageDDescriptorInspector，该脚本会继续运行 Release `RopeChunkEnumeratorBenchmarks --stage-d --include-alloc-stats` 与 Grapheme Telemetry harness/StageDTelemetry 烟雾，并把 `stage-d-inspector-latest.txt`、`chunk-bench-latest.txt`、`grapheme-telemetry-*.txt/.trx` 写入固定路径；Rust 默认 exporter 同步 `metric_windows[]` ledger（见 `tests/xi.Core.Tests/Fixtures/fixtures.manifest.json` 与 `[Fixture-MetricWindows]`），把 Ready Queue #3 交付的窗口计数直接暴露给 `[StageD::ParityAssets]`。同一批 artefact 驱动 Release chunk bench (chunk=255.93 MB/s，line=347.07 MB/s，alloc 37,944 B) 与 Grapheme telemetry (`tests/xi.Core.Tests/Fixtures/Reports/grapheme-telemetry-20251118T191914Z.txt` + `.trx`, ≥12,000 ops, fallback 0.30%)，并通过 `[QA-ChunkBench]` / `[QA-Telemetry]` / `[SO-StageDQAEvidence]` 链接到 `agents/information-researcher.md#证据快照` 的 evidence index，确保 Stage D → QA → Release → Evidence 的单一事实源。 | `[StageD::StageDChecklist]` · `[StageD::FixtureFlow]` · `[StageD::ParityAssets]` · `[Fixture-MetricWindows]` · `[QA-ChunkBench]` · `[QA-Telemetry]` · `[SO-StageDQAEvidence]` | Rust Porter + QA Engineer（`../../agents/rust-porter.md` · `../../agents/qa-engineer.md`） |
...
1. **Stage D Orchestration**：`[StageD::StageDChecklist]`→`[StageD::FixtureFlow]`→`[StageD::ParityAssets]` 负责记录 `python scripts/refresh_all_assets.py`（无参数）一键入口，该入口顺序执行 `rust-skeletons → dotnet-build → stage-d-fixtures → verify-stage-d → goal-tree → ilspy → Skeletonizer`，并在 `stage-d-fixtures` 子阶段完成 exporter、loader/hydrator smoke、manifest verifier、`StageDDescriptorInspector`、Release chunk bench、Grapheme Telemetry harness 与 StageDTelemetry TRX，确保所有 artefact 都落盘并写回 Goal Tree。需要局部 rerun 时再使用 `--only stage-d-fixtures`。 
...
- **`[StageD::FixtureFlow]`**：在 Sprint 1 中承接 Rust Porter 与 C# Implementer 的 exporter/loader/hydrator/inspector 命令，串起 `python scripts/refresh_all_assets.py`（无参数）+ `verify_fixture_manifest.py` + StageDDescriptorInspector，并把 manifest / CLI flag / `metric_windows[]` 结果写入唯一事实表，作为后续 QA 入口；必要时再通过 `--only stage-d-fixtures` 做局部 rerun。
