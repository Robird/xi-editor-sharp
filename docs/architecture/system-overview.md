...
| Stage D Diagnostics | Loader/Hydrator/Inspector 组合现已输出真实 Breaks/Diff/Search/Chunk/Grapheme ledger：`tests/xi.Core.Tests/Fixtures/fixtures.manifest.json`（`rust_commit=bd28ebdf83d2dd2fa6d4bd7c857d2471a21b4999`, `feature_gates=["cursor_state","serde"]`）与 `tests/xi.Core.Tests/Fixtures/Reports/stage-d-inspector-latest.txt` 同步记录计数/哈希。下一步是把这份证据写进 `[QA-IngestionSmoke]`/Goal Tree anchors，确保 QA/Stage D 引用单一事实来源。 | `[StageD::StageDChecklist]` · `[StageD::FixtureFlow]` · `[StageD::ParityAssets]` · `[RPM-Actions]` | Rust Porter + QA Engineer（`../../agents/rust-porter.md` · `../../agents/qa-engineer.md`） |
